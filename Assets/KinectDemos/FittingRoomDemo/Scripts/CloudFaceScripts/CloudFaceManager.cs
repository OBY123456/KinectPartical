...
//		}
  ...
  //      }
	...
	private void ProcessFaceError(UnityWebRequest www)
	{
		//ClientError ex = JsonConvert.DeserializeObject<ClientError>(www.text);
		ClientError ex = JsonUtility.FromJson<ClientError>(www.downloadHandler.ToString());
		...
		else
		{
			...
			ServiceError serviceEx = JsonUtility.FromJson<ServiceError>(www.downloadHandler.ToString());
